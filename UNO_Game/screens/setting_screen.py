...
class SettingScreen(Screen):
    def __init__(self, surface: Surface, manager: pygame_gui.UIManager, networking: Networking):
        ...
        self.resolution_menu = pygame_gui.elements.UIDropDownMenu(
            ...
            starting_option=str(config_data['resolution']['width']) +
            "x" + str(config_data['resolution']['height']),
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame_gui.UI_DROP_DOWN_MENU_CHANGED:
            if event.ui_element == self.resolution_menu:
                ...
                self.config_data["resolution"]["width"] = width
                self.config_data["resolution"]["height"] = height
        ...
        # 버튼 이벤트
            ...
            if event.ui_element == self.reset_button:
                self.config_data["resolution"]["width"] = 800
                self.config_data["resolution"]["height"] = 600
                with open('display_config.json', 'w') as f:
                    json.dump(self.config_data, f)
