...
class ServerLobby(Screen):
    ...
    def handle_event(self, event):
        ...
        #클라이언트로부터 슬롯 할당 메시지 받았을 때 처리하는 부분
        #self.received_slot = self.client_socket.recv(1024).decode()
        # print('슬롯' + self.received_slot + '이 할당되었습니다.')
        # # self.slot_connected[self.received_slot] = "connected"
        # 슬롯 할당 처리
        #self.button_states[self.received_slot] = "selected"
        ...
        if self.client_socket:
            try:
                ...
                if self.received_data:
                    print("받은 메세지:", self.received_data.decode())
