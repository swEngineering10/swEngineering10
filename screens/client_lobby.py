...
class ClientLobby(Screen):
    def __init__(self, surface: Surface, manager: pygame_gui.UIManager, networking: Networking):
        ...
        self.text_username = self.font_username.render(
            self.text_username_content, True, (255, 255, 255))
        self.text_username_width, self.text_username_height = self.font_username.size(
            self.text_username_content)
        ...
        self.text_complayer = self.font_complayer.render(
            self.text_complayer_content, True, (255, 255, 255))
        self.text_complayer_width, self.text_complayer_height = self.font_complayer.size(
            self.text_complayer_content)
        ...
        self.username_entry = pygame_gui.elements.UITextEntryLine(
            relative_rect=pygame.Rect((self.username_entry_x_pos, self.username_entry_y_pos), (
                self.username_entry_width, self.username_entry_height)),
            manager=manager
        )
        ...
        self.add_player_button_x_pos = self.screen_width // 2 - ((self.add_player_button_width * (
            self.add_player_button_count)) + (self.add_player_button_spacing * (self.add_player_button_count - 1))) // 2
        self.add_player_button_y_pos = self.screen_height // 2

        # Add Player 버튼 생성
        self.add_player_button_rects = []
        for i in range(self.add_player_button_count):
            button_rect = pygame.Rect(self.add_player_button_x_pos + i * (self.add_player_button_width + self.add_player_button_spacing),
                                      self.add_player_button_y_pos, self.add_player_button_width, self.add_player_button_height)
        ...
        self.active_index = 1
        self.button_states = ["inactive", "inactive",
        ...
        self.client_socket = self.networking.get_client_socket()
        '''
        from network_client import Client
        self.myInstance = Client(Surface, pygame_gui.UIManager, Networking)
        self.client_socket = self.myInstance.client_socket
        self.client_socket.sendall(self.slot_number.encode())
        '''
    # 버튼 상태 업데이트 함수
    ...
    def run(self, events: list[Event]) -> bool:
        ...
        self.screen.blit(self.text_username,
                         (self.text_username_x_pos, self.text_username_y_pos))
        self.screen.blit(self.text_complayer,
                         (self.text_complayer_x_pos, self.text_complayer_y_pos))
        ...
        if self.client_socket:
            self.client_socket.sendall(self.computer_number.encode())
            print("번호를 전송합니다:", self.computer_number)
